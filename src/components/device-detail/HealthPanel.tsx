...
// function calcCurrentFMB150(telemetry: TelemetryParam[]): number | null {
//   const raw = getRawIO(telemetry, 9);          // Ain.1, was incorrectly 6
//   if (raw === null) return null;
//   return parseFloat(((raw * 0.001 / 83) * 1000).toFixed(2));
// }
...
//   // LOGIC-ONLY flag. FMB150 hardware (other than EOW) always uses the MINI signal
//   // set — V/83 current and a single Ain.2 internal water level — regardless of how
//   // the device is named. This covers KVA-named gensets wired to an FMB150 (e.g.
//   // "GreenX 625KVA" on FMB150 IMEI 353201355105328): they get MINI health logic
//   // while keeping their real 625KVA branding in the header. Kept SEPARATE from
//   // `isMini` so the display never flips to "GreenDrive Mini".
//   const useMiniLogic = isFMB150 && deviceModel !== "EOW";

//   // Allow FMB150 to carry a KVA model name (it just maps to MINI logic above).
//   const validFMC650Models = ["380KVA", "625KVA", "1500KVA", "EOW"];
//   const validFMB150Models = ["MINI", "EOW", "380KVA", "625KVA", "1500KVA"];
//   if (isFMC650 && !validFMC650Models.includes(deviceModel)) return null;
//   if (isFMB150 && !validFMB150Models.includes(deviceModel)) return null;
//   if (!isFMC650 && !isFMB150) return null;
...
//             <SignalRow label="Output current" detail={isFMC650 ? "Ain.1 → (V / 47) × 1000" : "Ain.1 → V / 83"}
//               status={ain1A !== null ? `${ain1A} A` : "—"}
//               ok={useMiniLogic ? (ain1A !== null && ain1A > 2 ? miniHealthy : null) : ain1A !== null ? ain1A > 2 : null}
//               stale={stale || isDataBlocked} isMobile={isMobile} />
//             {/* {useMiniLogic && ain1A !== null && ain1A > 2 && (
//               <SignalRow label="Current health range" detail="Healthy: 9–11 A"
//                 status={miniHealthy ? "Within 9–11 A" : `Out of range (${ain1A} A)`}
//                 ok={miniHealthy} stale={stale || isDataBlocked} isMobile={isMobile} />
//             )} */}
...
// file has been dropped here for readability. This is the live version with
// FMB120 support added. Re-paste your legacy block on top if you want it kept.
...
// ─── FMB120 analog config ─────────────────────────────────────────────────────
// FMB120 is wired like the FMB150 MINI, but its analog front-end is what you
// flagged as "varying". These three values are the ONLY things that differ
// between FMB150 and FMB120 in this panel — everything else is shared logic.
//
//   • Current/CT input → IO 9 (Ain.1). CONFIRMED: FMB120 emits Analog Input 1
//     on AVL ID 9 (Teltonika data-sending params). Same slot as FMB150.
//   • Current divisor  → defaulted to 83 (same transducer as FMB150). CONFIRM
//     against real device current with the SQL query I gave you.
//   • Water input      → defaulted to IO 6 (same as FMB150). CONFIRM — if your
//     FMB120 units wire the level sensor to the standard Analog Input 2, that
//     is IO 10, not IO 6. Flip this one constant if so.
const FMB120_CURRENT_IO  = 9;
...
function calcCurrentFMB120(telemetry: TelemetryParam[]): number | null {
  ...
  if (raw === null) return null;
  return parseFloat(((raw * 0.001 / FMB120_CURRENT_DIV) * 1000).toFixed(2));
}
...
// ─── FMB-family alarm logic (FMB150 + FMB120) ─────────────────────────────────
// Was computeAlarmsFMB150(telemetry). Generalised so FMB120 reuses the exact
// same logic — only the current-calc function and the water IO differ, and both
// are passed in. FMB150 behaviour is unchanged (calcCurrentFMB150, water IO 6).
...
export function GreenXHealthPanel({ deviceType, deviceModel, telemetry }: GreenXHealthPanelProps) {
  ...
  // signal set — single-divisor current and a single Ain.2 internal water level —
  // regardless of how the device is named. This covers KVA-named gensets wired to
  // an FMB150/FMB120 (e.g. "GreenX 625KVA"): they get MINI health logic while
  // keeping their real KVA branding in the header. Kept SEPARATE from `isMini` so
  // the display never flips to "GreenDrive Mini".
  const useMiniLogic = (isFMB150 || isFMB120) && deviceModel !== "EOW";

  // Hardware label used only in the model badge ("Mini (FMB120)" vs "Mini (FMB150)").
  const hwLabel = isFMC650 ? "FMC650" : isFMB120 ? "FMB120" : "FMB150";

  // Allow FMB150/FMB120 to carry a KVA model name (it just maps to MINI logic above).
  const validFMC650Models = ["380KVA", "625KVA", "1500KVA", "EOW"];
  const validFMBModels    = ["MINI", "EOW", "380KVA", "625KVA", "1500KVA"];
  if (isFMC650 && !validFMC650Models.includes(deviceModel)) return null;
  if (isFMB150 && !validFMBModels.includes(deviceModel)) return null;
  if (isFMB120 && !validFMBModels.includes(deviceModel)) return null;
  if (!isFMC650 && !isFMB150 && !isFMB120) return null;
  ...
  const ain1A     = isFMC650 ? calcCurrentFMC650(telemetry)
                  : isFMB120 ? calcCurrentFMB120(telemetry)
  ...
  // ── FMB150 / FMB120 water display ─────────────────────────────────────────
  const fmbWaterIo     = isFMB120 ? FMB120_WATER_IO : 6;
  const ain2raw        = (isFMB150 || isFMB120) ? getRawIO(telemetry, fmbWaterIo) : null;
  const ain2V          = ain2raw !== null ? ain2raw * 0.001 : null;
  const miniWaterOk    = ain2V !== null ? ain2V <= 20 : null;
  const miniWaterLabel = ain2V === null ? "Unknown" : ain2V <= 20 ? "OK — full" : "Low — shortage";
  ...
  // Output-current detail string (per-hardware divisor shown to the user).
  const currentDetail = isFMC650 ? "Ain.1 → (V / 47) × 1000"
    : isFMB120 ? `Ain.1 → V / ${FMB120_CURRENT_DIV}`
    : "Ain.1 → V / 83";
        ...
        <>
          ...
          <div style={{ padding: isMobile ? "12px 14px" : "16px 20px", borderBottom: `1px solid ${C.border}`, background: C.white }}>
            ...
            <SignalRow label="System running" detail="Din.1 = ON & Ain.1 > 2 A"
              ...
              ok={isRunning ? true : din1 === null ? null : false}
              stale={stale || isDataBlocked} isMobile={isMobile} />
            <SignalRow label="Output current" detail={currentDetail}
              status={ain1A !== null ? `${ain1A} A` : "—"}
              ok={useMiniLogic ? (ain1A !== null && ain1A > 2 ? miniHealthy : null) : ain1A !== null ? ain1A > 2 : null}
              stale={stale || isDataBlocked} isMobile={isMobile} />
            {/* {useMiniLogic && ain1A !== null && ain1A > 2 && (
            ...
            {useMiniLogic && (
              <SignalRow label="Internal water level" detail={`Ain.2 (IO ${fmbWaterIo}) — cell body water (<20 V = full)`}
                status={miniWaterLabel} ok={miniWaterOk}
                stale={stale || isDataBlocked} isMobile={isMobile} />
            )}
            {isFMC650 && hasMainTankDevice && (
