...
export function CommandsTab({ device }: CommandsTabProps) {
  ...
  const handleSend = async () => {
    ...
    try {
      const response = await fetch("/api/commands", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ device_id: device.id, command: commandText }),
      });
      const data = await response.json();
      if (data.success) {
        setCommandText("");
  ...
  const statusColors: Record<string, { bg: string; color: string }> = {
    executed: { bg: "rgba(0, 200, 83, 0.15)", color: "#4ade80" },
    pending:  { bg: "rgba(251, 191, 36, 0.15)", color: "#fbbf24" },
    sent:     { bg: "rgba(59, 130, 246, 0.15)", color: "#60a5fa" },
    failed:   { bg: "rgba(239, 68, 68, 0.15)", color: "#f87171" },
    timeout:  { bg: "rgba(239, 68, 68, 0.15)", color: "#f87171" },
  };

  return (
    <div style={{ padding: isMobile ? 14 : 24 }}>
      <div style={{ fontSize: isMobile ? 15 : 16, fontWeight: 700, color: THEME.text.primary, marginBottom: 16 }}>
        Send Command
      </div>
      ...
      <div style={{
        ...
        <button
          ...
          disabled={loading || !commandText.trim()}
          style={{
            background: loading ? THEME.neutral[400] : "#7c3aed",
            border: "none",
            borderRadius: 8,
      ...
      <div style={{
        ...
        <Icons.Info /> Command will be queued and sent when device connects
      </div>

      {/* Command history */}
      <div style={{ fontSize: 13, fontWeight: 700, color: THEME.text.secondary, marginBottom: 10 }}>
        Command History
      </div>

      {commands.length === 0 && (
        <div style={{ color: THEME.text.tertiary, fontSize: 12, padding: 20, textAlign: "center" }}>
          No commands sent yet. Queue a command above.
        </div>
      )}

      {commands.map((cmd) => {
        const sc = statusColors[cmd.status] || statusColors.pending;
        return (
          <div
            key={cmd.id}
            style={{
          ...
          >
            <div style={{
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8, flex: 1, minWidth: 0 }}>
                ...
                <code style={{
                  fontSize: isMobile ? 12 : 13, color: "#7c3aed", fontWeight: 600,
                  overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap",
                }}>
                  {cmd.command}
                </code>
              </div>
              <span style={{ fontSize: 10, color: THEME.text.tertiary, flexShrink: 0 }}>
                {formatTimestamp(cmd.sent_at)}
              </span>
            </div>
            ...
            {cmd.response && (
              <div style={{
                ...
                background: "rgba(0,200,83,0.06)", padding: "6px 10px",
                borderRadius: 6, marginTop: 6, wordBreak: "break-all",
              }}>
                → {cmd.response}
              </div>
            )}
