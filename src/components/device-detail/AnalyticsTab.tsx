...
function DatetimeRangePicker({ startDate, startTime, endDate, endTime, onConfirm, onClose, isMobile }: {
  startDate: string|null; startTime: string;
  endDate: string|null; endTime: string;
  onConfirm: (sd:string, st:string, ed:string, et:string) => void;
  onClose: () => void;
  ...
  return (
    <div style={{
      position: isMobile ? "fixed" : "absolute" as any,
      ...(isMobile
        ? { top: 0, left: 0, right: 0, bottom: 0, zIndex: 9999, borderRadius: 0, width: "auto",
            display: "flex", flexDirection: "column" as const, justifyContent: "center", padding: 16,
            background: "rgba(0,0,0,0.5)" }
        : { top: "calc(100% + 8px)", right: 0, zIndex: 999, width: 290 }),
    }}>
      <div style={{
        background: "white", borderRadius: 14, border: `2px solid ${THEME.border.light}`,
        boxShadow: "0 8px 32px rgba(0,0,0,0.14)", padding: 16,
        ...(isMobile ? { maxHeight: "90vh", overflowY: "auto" as any } : {}),
      }}>
        {/* Step toggle */}
        <div style={{ display:"flex", gap:6, marginBottom:12 }}>
          {(["start","end"] as const).map(s => {
        ...
        {isMobile && (
          <button onClick={onClose} style={{
            ...
            fontSize: 13, fontWeight: 700, border: `1px solid ${THEME.border.light}`,
            background: THEME.neutral[50], color: THEME.text.secondary,
...
function SectionHeader({ icon, title, sub, isMobile }: { icon:string; title:string; sub:string; isMobile?:boolean }) {
  return (
  ...
  const renderComparisonChart = () => {
    ...
    return (
      <div style={{width:"100%",overflowX:"auto",WebkitOverflowScrolling:"touch" as any}}>
        ...
        <div style={{display:"flex",gap: isMobile ? 12 : 18,marginTop:10,paddingLeft:PAD.left, flexWrap:"wrap"}}>
          <div style={{display:"flex",alignItems:"center",gap:6}}>
            <div style={{width:18,height:3,background:THEME.primary[500],borderRadius:2}}/>
            <span style={{fontSize:11,color:THEME.text.secondary}}>Actual</span>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:6}}>
            <div style={{width:18,height:2,borderTop:"2px dashed #f59e0b"}}/>
            <span style={{fontSize:11,color:THEME.text.secondary}}>Baseline ({baselineKmpl})</span>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div style={{ padding: isMobile ? 14 : 24, background:THEME.background.secondary, minHeight:"100%", overflowY:"auto" }}>
      ...
      <div style={{
        ...
        {/* Controls: preset buttons + custom picker */}
        <div style={{ display:"flex", gap: isMobile ? 6 : 6, alignItems:"center" }}>
          <div style={{ display:"flex", gap:2, background:THEME.neutral[100], padding:3, borderRadius:8 }}>
            {[1,7,14].map(d=>(
              <button key={d} onClick={()=>{setCustomMode(false);setDays(d);}} style={{
                padding: isMobile ? "5px 12px" : "7px 18px", borderRadius:6, fontSize: isMobile ? 11 : 12, fontWeight:700, cursor:"pointer",
                fontFamily:"inherit", border:"none", transition:"all 0.15s",
                background:!customMode&&days===d?THEME.primary[500]:"transparent",
                color:!customMode&&days===d?"white":THEME.text.secondary,
                boxShadow:!customMode&&days===d?THEME.shadow.sm:"none" }}>{d}D</button>
          ...
          <div ref={calendarRef} style={{position:"relative"}}>
            <button onClick={()=>setCalendarOpen(o=>!o)} style={{
              padding: isMobile ? "5px 8px" : "7px 13px", borderRadius:8, fontSize: isMobile ? 11 : 12, fontWeight:700, cursor:"pointer",
              fontFamily:"inherit", border:"none", transition:"all 0.15s",
              background:customMode?`linear-gradient(90deg,${THEME.primary[500]},#8b5cf6)`:THEME.neutral[100],
              color:customMode?"white":THEME.text.secondary,
              boxShadow:customMode?THEME.shadow.sm:"none",
              display:"flex", alignItems:"center", gap:4, maxWidth: isMobile ? 130 : 230, overflow:"hidden" }}>
              <span style={{ fontSize: isMobile ? 12 : 14 }}>📅</span>
              <span style={{overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth: isMobile ? 80 : 180}}>
                {customLabel}
              </span>
      ...
      <div style={{ display:"flex", flexDirection: isMobile ? "column" : "row",
        ...
        <SectionHeader icon="⛽" title="Fuel Efficiency" sub="Daily consumption vs baseline target" isMobile={isMobile} />
        {customMode&&startDate&&endDate&&(
          <div style={{fontSize:11,fontWeight:700,color:THEME.primary[600],
            background:THEME.primary[50],border:`1px solid ${THEME.primary[200]}`,
            borderRadius:8,padding:"4px 10px",marginBottom:16,whiteSpace:"nowrap",
            overflow:"hidden",textOverflow:"ellipsis",maxWidth: isMobile ? "100%" : "auto"}}>
            📅 {formatDatetimeLabel(startDate,startTime,endDate,endTime)}
          </div>
        )}
      </div>
      ...
      {!fuelLoading&&summary&&(
        <div style={{ display:"grid", gridTemplateColumns:statGrid, gap: isMobile ? 8 : 12, marginBottom:16 }}>
          <StatCard icon="🛣️" label="Total Distance"
            value={`${summary.total_distance_km.toLocaleString("en-IN")} km`}
            sub={`${summary.days_with_data} days`} color={THEME.primary[500]} bg={THEME.primary[50]} isMobile={isMobile}/>
          <StatCard icon="⛽" label="Fuel Consumed"
            value={`${summary.total_fuel_litres.toLocaleString("en-IN")} L`}
            sub={windowSub} color="#f59e0b" bg="#fffbeb" isMobile={isMobile}/>
          <StatCard icon="📈" label="Avg Efficiency"
            value={summary.overall_fuel_average_kmpl?`${summary.overall_fuel_average_kmpl} km/L`:"N/A"}
            sub={`Baseline: ${baselineKmpl}`} color="#3b82f6" bg="#eff6ff" isMobile={isMobile}/>
          <StatCard icon={improved?"🌱":"⚡"} label="CO₂ Impact"
            value={validDays.length>0?`${improved?"-":"+"}${co2Saved} kg`:"N/A"}
            sub={improved?`${Math.abs(fuelSaved)}L saved`:`${Math.abs(fuelSaved)}L extra`}
            color={improved?"#16a34a":"#dc2626"} bg={improved?"#dcfce7":"#fef2f2"} isMobile={isMobile}/>
        </div>
      )}
      {fuelLoading&&<Spinner/>}

      <Card style={{ marginBottom:16, padding: isMobile ? 14 : 24 }}>
        <div style={{
          display:"flex", flexDirection: isMobile ? "column" : "row",
          justifyContent:"space-between", alignItems: isMobile ? "flex-start" : "center",
          gap: isMobile ? 10 : 0, marginBottom:18,
        }}>
          <div>
            <div style={{ fontSize: isMobile ? 13 : 14, fontWeight:700, color:THEME.text.primary }}>Actual vs Baseline</div>
            <div style={{ fontSize:11, color:THEME.text.tertiary, marginTop:2 }}>km/L — higher is better</div>
          </div>
          {editingBaseline?(
            ...
            <button onClick={()=>setEditingBaseline(true)} style={{padding:"6px 14px",background:"#fffbeb",
              border:"2px solid #f59e0b",borderRadius:8,fontSize: isMobile ? 12 : 13,fontWeight:700,color:"#92400e",
              cursor:"pointer",fontFamily:"inherit"}}>✏️ {baselineKmpl} km/L</button>
          )}
        </div>
        {fuelLoading?<Spinner/>:renderComparisonChart()}
      </Card>
      ...
      {!fuelLoading&&validDays.length>0&&(
        <div style={{
          ...
          background:improved?"linear-gradient(135deg,#f0fdf4,#dcfce7)":"linear-gradient(135deg,#fef2f2,#fee2e2)",
          border:`2px solid ${improved?"#86efac":"#fca5a5"}`,
          borderRadius:14, padding: isMobile ? "14px 16px" : "18px 22px", marginBottom:28, boxShadow:"0 1px 4px rgba(0,0,0,0.05)" }}>
          <span style={{ fontSize: isMobile ? 32 : 40 }}>{improved?"🌱":"⚠️"}</span>
          <div style={{flex:1}}>
            <div style={{ fontSize: isMobile ? 13 : 15, fontWeight:800, color:improved?"#15803d":"#b91c1c" }}>
              {improved?`${co2Saved} kg CO₂ saved`:`${co2Saved} kg extra CO₂`}
            </div>
            <div style={{ fontSize: isMobile ? 11 : 12, color:improved?"#166534":"#991b1b", marginTop:3 }}>
              {improved?`${Math.abs(fuelSaved)}L less fuel than baseline`
                :`${Math.abs(fuelSaved)}L more fuel than baseline`}
            </div>
          </div>
          <div style={{ textAlign: isMobile ? "left" : "right" }}>
            ...
            <div style={{ fontSize: isMobile ? 18 : 22, fontWeight:800, color:improved?"#16a34a":"#dc2626" }}>
              {improved?"-":"+"}{Math.abs(fuelSaved)} L
            </div>
          </div>
        </div>
      ...
      {/* ── SECTION 2: TRIPS ── */}
      <SectionHeader icon="🗺️" title="Trip Analysis" sub="Ignition ON → OFF trips with GPS distance" isMobile={isMobile} />
      {tripsLoading?<Spinner/>:tripSummary&&(
        <>
          <div style={{ display:"grid", gridTemplateColumns:statGrid, gap: isMobile ? 8 : 12, marginBottom:16 }}>
            <StatCard icon="🚗" label="Total Trips" value={`${tripSummary.total_trips}`}
              sub={windowSub} color="#8b5cf6" bg="#f5f3ff" isMobile={isMobile}/>
            <StatCard icon="📍" label="GPS Distance" value={`${tripSummary.total_distance_km} km`}
              sub="Haversine" color={THEME.primary[500]} bg={THEME.primary[50]} isMobile={isMobile}/>
            <StatCard icon="⏱️" label="Drive Time" value={formatDuration(tripSummary.total_duration_min)}
              sub={`Avg ${formatDuration(tripSummary.avg_trip_duration_min)}/trip`} color="#0891b2" bg="#ecfeff" isMobile={isMobile}/>
            <StatCard icon="🏆" label="Longest Trip" value={`${tripSummary.longest_trip_km} km`}
              sub={`Avg ${tripSummary.avg_trip_distance_km} km`} color="#f59e0b" bg="#fffbeb" isMobile={isMobile}/>
          </div>

          {trips.length>0&&(
            <Card style={{ marginBottom:28, padding: isMobile ? 14 : 24 }}>
              <div style={{fontSize:13,fontWeight:700,color:THEME.text.primary,marginBottom:14}}>Trip Log</div>
                    ...
                    <div key={trip.trip_number} style={{
                      ...
                      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:6 }}>
                        <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                          <div style={{ width:24, height:24, borderRadius:"50%", background:THEME.primary[100],
                            color:THEME.primary[600], display:"flex", alignItems:"center", justifyContent:"center",
                            fontSize:10, fontWeight:800 }}>{trip.trip_number}</div>
                          <span style={{ fontSize:12, fontWeight:600, color:THEME.text.primary }}>
                            {formatTime(trip.start_time)} → {formatTime(trip.end_time)}
                          </span>
                        </div>
                        <span style={{ fontSize:12, fontWeight:700, color:THEME.primary[600] }}>{trip.distance_km} km</span>
                      </div>
                      <div style={{ display:"flex", gap:12, fontSize:11, color:THEME.text.tertiary }}>
                        <span>⏱ {formatDuration(trip.duration_min)}</span>
                        <span>⌀ {trip.avg_speed_kmh} km/h</span>
                        <span>↑ {trip.max_speed_kmh} km/h</span>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          {trips.length===0&&(
            <Card style={{marginBottom:28,textAlign:"center",color:THEME.text.tertiary,fontSize:13,padding:32}}>
              No trips detected in this period.
            </Card>
          )}
        </>
      )}
      ...
      {/* ── SECTION 3: IDLE ── */}
      <SectionHeader icon="💤" title="Idle Time" sub="Engine ON + Speed = 0 for 5+ minutes" isMobile={isMobile} />
      {idleLoading?<Spinner/>:idleSummary&&(
        <>
          <div style={{ display:"grid", gridTemplateColumns:statGrid, gap: isMobile ? 8 : 12, marginBottom:16 }}>
            <StatCard icon="⏳" label="Total Idle" value={formatDuration(idleSummary.total_idle_minutes)}
              sub={`${idleSummary.total_idle_events} events`} color="#dc2626" bg="#fef2f2" isMobile={isMobile}/>
            <StatCard icon="📊" label="Avg Idle" value={formatDuration(idleSummary.avg_idle_duration_min)}
              sub="Per event" color="#f59e0b" bg="#fffbeb" isMobile={isMobile}/>
            <StatCard icon="⛽" label="Fuel Wasted" value={`${idleSummary.estimated_fuel_wasted_litres} L`}
              sub={idleSummary.idle_fuel_rate_source === "can_bus"
                ? `@ ${idleSummary.idle_fuel_rate_lph} L/h (CAN)`
                : `@ ${idleSummary.idle_fuel_rate_lph ?? 0.8} L/h (est.)`}
              color="#ea580c" bg="#fff7ed" isMobile={isMobile}/>
            <StatCard icon="🌫️" label="CO₂ Idling" value={`${idleSummary.estimated_co2_wasted_kg} kg`}
              sub="Preventable" color="#7c3aed" bg="#f5f3ff" isMobile={isMobile}/>
          </div>

          {dailyIdle.length>0&&(
            <Card style={{ marginBottom:24, padding: isMobile ? 14 : 24 }}>
              <div style={{fontSize:13,fontWeight:700,color:THEME.text.primary,marginBottom:14}}>Daily Idle</div>
              {dailyIdle.map((row,i)=>(
                <div key={row.day} style={{
                  ...
                  <div style={{ fontSize:12, fontWeight:600, color:THEME.text.primary,
                    minWidth: isMobile ? 70 : 120, flexShrink:0 }}>
                    {isMobile
                  ...
                  <div style={{ fontSize:12, fontWeight:700, color:"#dc2626", textAlign:"right",
                    minWidth: isMobile ? 50 : 90, flexShrink:0 }}>
                    {formatDuration(row.idle_minutes)}
                  </div>
                  {!isMobile && (
                    <div style={{fontSize:11,color:THEME.text.tertiary,textAlign:"right",minWidth:80,flexShrink:0}}>
                      {row.idle_events} event{row.idle_events!==1?"s":""}
                    </div>
                  )}
                </div>
