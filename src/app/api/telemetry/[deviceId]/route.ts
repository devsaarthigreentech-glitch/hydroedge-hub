...
//   47:  { name: "can.security.state.flags",      category: "can" },
...
//   141: { name: "can.battery.temperature",       unit: "°C",  category: "can", multiplier: 0.1 },
//   142: { name: "can.battery.level.percent",     unit: "%",   category: "can" },
//   143: { name: "can.door.status.bitmask",       category: "can" },
...
// ─── Shared IO map (same meaning on ALL Teltonika devices) ───────────────────
...
const SHARED_IO_MAP: Record<number, IOParam> = {
  ...
  // GNSS quality (IO records — multiplier needed)
  181: { name: "position.pdop",                    category: "gps",         multiplier: 0.1 },
  182: { name: "position.hdop",                    category: "gps",         multiplier: 0.1 },
  ...
  24:  { name: "can.vehicle.speed",                unit: "km/h",    category: "can" },
  25:  { name: "can.engine.temperature",           unit: "°C",      category: "can",         multiplier: 0.1 },
  30:  { name: "can.vehicle.speed.adapter",        unit: "km/h",    category: "can" },
  31:  { name: "can.throttle.pedal.level",         unit: "%",       category: "can" },
  33:  { name: "can.fuel.consumed",                unit: "l",       category: "can",         multiplier: 0.1 },
  34:  { name: "can.fuel.level.liters",            unit: "l",       category: "can",         multiplier: 0.1 },
  35:  { name: "can.engine.rpm",                   unit: "rpm",     category: "can" },
  ...
  176: { name: "can.dtc.errors",                   category: "can" },
  949: { name: "can.engine.rpm.alt",               unit: "rpm",     category: "can" },
  1130: { name: "can.engine.rpm.lv",               unit: "rpm",     category: "can" },
};

// ─── FMB150-specific IDs ──────────────────────────────────────────────────────
// Only IDs that are DIFFERENT or EXCLUSIVE to FMB150
...
const FMB150_ONLY: Record<number, IOParam> = {
  ...
  113: { name: "battery.level",                    unit: "%",   category: "power" },
  // Analog Input 2 (FMB150 uses ID 6)
  6:   { name: "ain.2",                            unit: "V",   category: "analog", multiplier: 0.001 },
  // SD card / SIM (FMB150 IDs)
  10:  { name: "sd.card.status",                   category: "system" },
  11:  { name: "sim.iccid1",                       category: "system" },
  237: { name: "network.type",                     category: "system" },
  // OBD elements (FMB150 OBD section — different from CAN adapter IDs)
  112: { name: "can.engine.temperature.obd",       unit: "°C",  category: "can" },
  114: { name: "can.engine.rpm.obd",               unit: "rpm", category: "can" },
  115: { name: "can.throttle.pedal.level.obd",     unit: "%",   category: "can" },
  116: { name: "can.coolant.temperature.obd",      unit: "°C",  category: "can" },
  // Fuel GPS (FMB150 specific IDs)
  4:   { name: "pulse.counter.din1",               category: "digital" },
  5:   { name: "pulse.counter.din2",               category: "digital" },
};

// ─── FMC650-specific IDs ──────────────────────────────────────────────────────
// Only IDs that are DIFFERENT or EXCLUSIVE to FMC650
...
const FMC650_ONLY: Record<number, IOParam> = {
  ...
  237: { name: "accelerometer.y",                  unit: "mG",  category: "motion" },
  238: { name: "accelerometer.z",                  unit: "mG",  category: "motion" },
  // Analog Inputs 2-4 (FMC650 uses different IDs)
  10:  { name: "ain.2",                            unit: "V",   category: "analog", multiplier: 0.001 },
  11:  { name: "ain.3",                            unit: "V",   category: "analog", multiplier: 0.001 },
  245: { name: "ain.4",                            unit: "V",   category: "analog", multiplier: 0.001 },
  // Digital extras
...
export async function GET(
  ...
  try {
    ...
    // GPS query with sanity bounds to reject corrupt records
    const gpsResult = await query(
      `SELECT 
        latitude, longitude, altitude, speed,
        heading as direction, satellites, hdop,
      ...
      WHERE device_id = $1
        ...
        AND speed     BETWEEN 0    AND 500
        AND satellites BETWEEN 0   AND 100
        AND altitude  BETWEEN -500 AND 9000
      ORDER BY timestamp DESC
      LIMIT 1`,
      [deviceId]
    );
    ...
    const sysTs = gpsData?.timestamp || new Date().toISOString();
    telemetryParams.push(
      { id: 0, name: "ident",       value: device.imei,                                    type: "system", category: "system", timestamp: sysTs },
      { id: 1, name: "device.name", value: device.device_name || "Unknown",                type: "system", category: "system", timestamp: sysTs },
      { id: 2, name: "device.type", value: `${device.manufacturer} ${device.device_type}`, type: "system", category: "system", timestamp: sysTs }
    );

    // ── GPS params (already real-world values from DB, NO multiplier) ──────
    if (gpsData) {
      const gpsFields: [string, any, string?][] = [
        ["position.latitude",   gpsData.latitude],
        ["position.longitude",  gpsData.longitude],
    ...
    for (const io of ioData) {
      ...
      telemetryParams.push({
        ...
        name:      mapping ? mapping.name : `io.${ioId}`,
        value:     displayValue,
        raw_value: mapping?.multiplier ? rawValue : undefined,
        unit:      mapping?.unit || "",
        type:      "sensor",
        category:  mapping?.category || "unknown",
        timestamp: io.timestamp,
