...
export async function GET(
  ...
  try {
    ...
    // 4. Distinct asset types for pill filter
    const assetResult = await query(
    ...
    return NextResponse.json({
      ...
      data: {
        ...customer,
        stats: {
          total_devices:  parseInt(statsResult.rows[0].total_devices)  || 0,
          online_devices: parseInt(statsResult.rows[0].online_devices) || 0,
          sub_customers:  parseInt(subResult.rows[0].sub_count)        || 0,
        },
        asset_types: assetResult.rows.map((r) => r.asset_type),
      },
    });
  ...
  try {
    ...
    if (body.email) {
      ...
      if (emailCheck.rows.length > 0) {
        return NextResponse.json({ success: false, error: "Email already in use by another customer" }, { status: 400 });
      }
    }
